...
- Interpret these coefficients (keep in mind Neighborhood is not shown)!
- Do we think that the coefficient on Black is likely to be biased up or down?

```{r}
etable(m1, m2, m3, 
             keep = c('BlackTRUE', 'problemtraffic', 'BlackTRUE:problemtraffic'))
```
...
- Pretty different! Let's use robust SEs (note we can do these after the fact in `etable()` directly)
...
```{r}
m1 <- feols(gift ~ mailsyear, data = charity)
m2 <- feols(gift ~ mailsyear + propresp + giftlast, data = charity)
m3 <- feols(gift ~ mailsyear + giftlast + avggift, data = charity)
etable(m1,m2,m3)
```
