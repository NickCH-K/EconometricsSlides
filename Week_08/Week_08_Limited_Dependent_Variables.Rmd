...
We can calculate the individual marginal effects using `marginal_effects` and the MER and AME easily using `margins` in the **margins** package (`margins` can also be used to make MEM but you basically just calculate the means yourself and use that for a MER)
...
There are other marginal effects functions that all work roughly similarly to each other - use `logitmfx()` and `probitmfx()` in **mfx** if you must! But I do recommend **margins**

---

# Marginal Effects in R, Take 2

```{r, echo = TRUE}
library(mfx)
# MEM by default, so we must set atmean = FALSE, and re-tell it our data
logitfx <- logitmfx(marriedlogit, atmean = FALSE, data = gss_cat)
export_summs(logitfx, digits = 4, statistics = c(N = 'nobs.1'))
```
