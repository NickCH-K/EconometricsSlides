...
We can calculate the individual marginal effects using `marginaleffects()` and the MER and AME easily using `summary(marginaleffects())` in the **marginaleffects** package. 
...
# AME below
summary(marginaleffects(marriedlogit))
```
...
summary(marginaleffects(marriedlogit, datagrid(age  =50, tvhours = 2, grid.type = 'counterfactual')))
```
