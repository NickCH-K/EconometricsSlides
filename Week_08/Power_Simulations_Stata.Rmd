...
`rnormal()` takes two arguments: the mean and the standard deviation. `rnorm(0, 3)` produces random normal draws from a normal distribution with a mean of 0 and a standard deviation of 3.

`runiform()` takes arguments: the start and the end of the distribution. `runif(1, 3)` produces random uniform draws from a uniform distribution that goes from 1 to 3.
