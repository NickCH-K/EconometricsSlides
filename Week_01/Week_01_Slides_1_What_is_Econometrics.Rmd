...
- Homework is graded on **timely completion** alone. Suggested revision, followed by **weekly quizzes** on that same material the following week. Lowest two quizzes dropped, including absences outside extraordinary circumstances
- There is a cumulative final
...
ggplot(tibble(x = c(1:100,1:100), y = c(100:1, 1:100), linet = c(rep('Theoretical D',100),rep('Theoretical S',100))),
  ...
  geom_dl(aes(label = linet, x = x + 2), method = 'maxvar.points', size = 16/.pt) + 
...
2. $Y \rightarrow X$ (Y causes X)?
3. $X \leftarrow Z \rightarrow Y$ (Z causes both X and Y)?
4. $X \rightarrow Z \rightarrow Y$ (X causes Z which causes Y)?
5. $Y \rightarrow Z \rightarrow X$ (Y causes Z which causes X)?
6. $X \leftarrow Z \leftarrow Y$ (Y causes Z which causes X)?
7. $X \rightarrow Y$ and $X \leftarrow Z \rightarrow Y$
8. Some other combination of two above?
...
- All of those are possible, and indistinguishable in the data (especially if we can't measure Z)
- If 3 or 6 are correct, we see the negative relationship even though minimum wage has *nothing to do* with causing employment (like the ice cream and shorts example)
- If 7 is correct, then even though we know minimum wage causes employment to change, the *size* or even *direction* of the relationship will be wrong (why?)
