...
- Let's return to our play-around code:
...
- Let's move things around. How does it change? 
- With smaller standard errors, can we get farther from the truth by just rerunning the code to get a new sample?
...
feols(ClassGrade ~ StudyTime, data = tb)
```
...
- We can do this in `feols` with `vcov = 'hetero'`
- Let's also start viewing our regression results in table format with **fixest**'s `etable()` while we're at it
...
- But for clustering we can just use *cluster-robust standard errors*. We just need to also give it the variable that we think the errors are clustered in, again in `vcov` with `~variable`

```{r, echo = TRUE}
data(Orange)
treemodel <- feols(circumference~age, data = Orange, vcov = ~Tree)
etable(treemodel)
```
