...
- What if $Temperature$ *weren't* in $\varepsilon$? Then we'd be fine! How do we take it out? Just put it in the model directly!
...
```{r, echo = TRUE}
tib <- tibble(U1 = rnorm(1000), A = rnorm(1000), B = rnorm(1000)) %>%
...
```{r}
m2 <- lm(Y~X + A + C, data = tib)
m3 <- lm(Y~X + A + D, data = tib)
